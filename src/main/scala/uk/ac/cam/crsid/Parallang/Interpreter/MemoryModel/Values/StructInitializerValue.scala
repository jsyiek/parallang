...
class StructInitializerValue(val argNames: List[String], val body: mutable.HashMap[String, Value] => Value) extends Value {
  ...
  override def deepCopy(): Value = throw new RuntimeException("A StructInitializerValue should NOT be passed as a message!")
